...
export class DAG<P, T extends Node<P>> {
  ...
  public order(
    ...
    //   const baseDist = distances.get(id) ?? 0;

    //   for (const neighbor of subdag.getOutEdges(id)) {
    //     const weight = subdag.edgeWeights.get(id)?.get(neighbor) ?? 1;
    //     const newDist = baseDist + weight;
    //     const prevDist = distances.get(neighbor) ?? Infinity;
    //     if (newDist < prevDist) {
